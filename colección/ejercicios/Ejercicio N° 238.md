...
1. Demostrar que $x_d(n)$ es periódica si y sólo si $\frac{T_0}{T_s}$ es un [[Número racional|número racional]]  ^parte-1

2. Determinar el período y la [[ingeniería electrónica/analisis 3/Serie de Fourier/Función periódica#Frecuencia|frecuencia]] fundamental de $x_d(n)$ cuando ésta es periódica. Expresar la frecuencia fundamental como una fracción de $T_0$ ^parte-2

 3. ¿Cuántos periodos de $T_0$ se necesitan para obtener las muestras que forman un solo período de $x_d(n)$ en el caso que ésta última es periódica? ^parte-3

# Resolución
---
[[colección/ejercicios/Ejercicio N° 238#^punto-1|1.]] Para poder decir que $\frac{T_0}{T_s}$ es un número racional, debemos llegar a una expresión tal que $$ \frac{T_0}{T_s} = \frac{a}{b}, ~ \text{con} ~ a, ~ b \in \mathbb{Z}, ~ \text{donde} ~ b \ne 0 $$
...
[[colección/ejercicios/Ejercicio N° 238#^punto-2|2.]] Como mencionábamos en el punto anterior, $x_d(n) = x_d(n + N)$ donde $N$ es el período de $x_d(n)$, y encontramos que $$ \frac{T_0}{T_s} = \frac{N}{k} $$
...
[[colección/ejercicios/Ejercicio N° 238#^punto-3|3.]] Sabemos que se cumple la relación $$ \frac{T_0}{T_s} = \frac{N_0}{k} $$ donde $T_0$ es el periodo de $x(t)$, $T_s$ es la distancia entre los puntos de la señal $x_d(n)$, $N_0$ es el periodo fundamental de $x_d(n)$ y $k$ es un número natural.
