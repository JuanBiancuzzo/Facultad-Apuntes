function crearEstructuraFacultad(tp) {
    const { 
        TAGS: { facultad: TAGS_FACULTAD, nota: TAGS_NOTA }, 
        DATOS: { ARCHIVO: DATOS_ARCHIVO, CARRERA: DATOS_CARRERA } 
    } = tp.user.constantes();
    const dv = app.plugins.plugins.dataview.api;
    const seccionCarrera = tp.user.seccionCarrera(tp).clase;
    const seccionMateria = tp.user.seccionMateria(tp).clase;
    const seccionResumen = tp.user.seccionResumenMateria(tp).clase;
    const tagsSeccion = tp.user.tagsSeccion(tp);

    let arrayCarreras = new Array(dv.pages(`#${TAGS_FACULTAD.self}/${TAGS_FACULTAD.carrera}`).length), indCarreras = 0; 
    let arrayMaterias = new Array(dv.pages(`#${TAGS_FACULTAD.self}/${TAGS_FACULTAD.materia}`).length), indMaterias = 0;
    let arrayResumenes = new Array(dv.pages(`#${TAGS_FACULTAD.self}/${TAGS_FACULTAD.resumen}`).length), indResumnes = 0;

    console.log(arrayCarreras.length, arrayMaterias.length, arrayResumenes.length);

    dv.pages(`#${TAGS_FACULTAD.self} and -#${TAGS_NOTA.self}`)
        .forEach(archivo => {
            let tagRepresentante = archivo[DATOS_ARCHIVO.tags]
                .filter(tag => tag.startsWith(TAGS_FACULTAD.self))
                .first()
                .replace(`${TAGS_FACULTAD.self}/`, "");

            let tags = archivo.tags.filter(tag => !tagsSeccion.some(tagSeccion => tag.includes(tagSeccion)) );

            switch (tagRepresentante) {
                case TAGS_FACULTAD.carrera: 
                    arrayCarreras[indCarreras] = {
                        tags, clase: new seccionCarrera({ [DATOS_CARRERA.nombre]: archivo.file.name, ...archivo })
                    };
                    indCarreras++;
                    break; 
                case TAGS_FACULTAD.materia: 
                    arrayMaterias[indMaterias] = {
                        tags, clase: new seccionMateria(archivo)
                    };
                    indMaterias++;
                    break; 
                case TAGS_FACULTAD.resumen: 
                    arrayResumenes[indResumnes] = {
                        tags, clase: new seccionResumen(archivo)
                    };
                    indResumnes++;
                    break;

                default:
                    throw Error(`De alguna forma el archivo no es carrera, materia o resumen, es:\n${archivo.file.name}`);
            }
        });

    let resultado = new Array(indCarreras + indMaterias + indResumnes), indResultado = 0;
    for (let i = 0; i < indCarreras; i++, indResultado++) { resultado[indResultado] = arrayCarreras[i]; }
    for (let i = 0; i < indMaterias; i++, indResultado++) { resultado[indResultado] = arrayMaterias[i]; }
    for (let i = 0; i < indResumnes; i++, indResultado++) { resultado[indResultado] = arrayResumenes[i]; }
    return resultado;

    let carreras = dv.pages(`#${TAGS_FACULTAD.self}/${TAGS_FACULTAD.carrera}`)
        .map(carrera => {
            // Creamos las carreras
            return seccionCarrera.clase({
                [DATOS_CARRERA.nombre]: carrera.file.name,
                ...carrera,
            });
        });

    return carreras;
}

/**
 * dv.pages(`#${this.tags.self}/${this.tags.materia} and #${this.tags.carrera}/${this.tagPorNombre(this.nombre.toLowerCase())}`)
            .forEach(materia => this.materias.push(this.informacion.materiaNueva(materia)));
 * 
 * 
 */

async function crear(tp) {
    const { DIRECTORIOS, TAGS: { facultad: TAGS_FACULTAD }, DATOS: { CARRERA: DATOS_CARRERA } } = tp.user.constantes();

    const dv = app.plugins.plugins.dataview.api;
    const tArchivo = tp.file.find_tfile(tp.file.path(true));
    const seccionCarrera = tp.user.seccionCarrera(tp);

    let carpeta = tArchivo.parent.path.split("/");
    if (carpeta.last().trim() == DIRECTORIOS.imagenes) carpeta.pop();

    console.log(crearEstructuraFacultad(tp));
    throw Error("Upsi")

    let directorioActual = carpeta.join("/");
    let cantidadChequeos = { numero: 0 };
    let mejorAjuste = carreras.flatMap(carrera => carrera.eleccionMasPrecisa(directorioActual, cantidadChequeos));

    console.log(cantidadChequeos);

    console.log("Mejor ajuste:")
    console.log(mejorAjuste);

    throw Error("Upsi")

    /*
    DATOS: { ARCHIVO: DATOS_ARCHIVO, MATERIA: DATOS_MATERIA, RESUMEN: DATOS_RESUMEN },
    const seccionMateria = tp.user.seccionMateria(tp);
    const seccionResumen = tp.user.seccionResumenMateria(tp);

    const preguntar = tp.user.preguntar();
    const obtenerTag = tp.user.obtenerTag;
    const error = tp.user.error();

    const carreras = dv.pages(`"${directorioActual}" and #${TAGS_FACULTAD.self}/${TAGS_FACULTAD.carrera}`);
    if (carreras.length > 1) {
        return await seccionCarrera.crear();
    }

    const materias = dv.pages(`"${directorioActual}" and #${TAGS_FACULTAD.self}/${TAGS_FACULTAD.materia}`);
    if (materias.length > 1 || carreras.length == 1) {
        // Buscamos obtener los tags de las materias, y sacarle el ultimo directorio, 
        // para obtener las carreras involucradas
        let tagsCarreras = materias
            .flatMap(materia => obtenerTag(tp, materia[DATOS_ARCHIVO.tags]))
            .filter(tag => tag.split("/").length > 1)
            .map(tag => "#" + tag.split("/").slice(0, -1).join("/"))
            .distinct();

        let carrerasPorMateria = [];
        if (tagsCarreras.length > 0) {
            carrerasPorMateria = dv.pages(`(${tagsCarreras.join(" or ")}) and #${TAGS_FACULTAD.self}/${TAGS_FACULTAD.carrera}`);
        }

        if (carreras.length > 0 && carrerasPorMateria.every(carrera => carrera.file.path != carreras.first().file.path)) {
            carrerasPorMateria = [ ...carrerasPorMateria, carreras.first()];
        }

        if (carrerasPorMateria.length == 0) 
            throw error.Quit("De alguna forma hay materias que no llevan a ninguna carrera");

        let carrera = carrerasPorMateria.first();
        if (carrerasPorMateria.length > 1) {
            carrera = await preguntar.suggester(
                tp, carrerasPorMateria.map(carrera => carrera.file.name), carrerasPorMateria,
                "En que carrera queres crear la materia?",
                error.Quit("No se ingreso que carrera se usa para crear la materia")
            )
        }

        return await seccionMateria.crear(carrera);
    }

    let resumenes = dv.pages(`"${directorioActual}" and #${TAGS_FACULTAD.self}/${TAGS_FACULTAD.resumen}`);
    if (resumenes.length > 0 || materias.length == 1) {
        // Buscamos obtener los tags de los resumenes, y sacarle el ultimo directorio, 
        // para obtener las materias involucradas
        let tagsMateria = resumenes
            .flatMap(resumen => obtenerTag(tp, resumen[DATOS_ARCHIVO.tags]).map(tag => {
                if (!resumen[DATOS_RESUMEN.parte]) return tag;

                let parte = resumen[DATOS_RESUMEN.parte];
                let { index } = [...tag.matchAll(`/${parte}`)].last();
                return tag.slice(0, index);
            }))
            .filter(tag => tag.split("/").length > 1)
            .map(tag => "#" + tag.split("/").slice(0, -1).join("/"))
            .distinct();

        let materiasPorResumen = [];
        if (tagsMateria.length > 0) {
            materiasPorResumen = dv.pages(`(${tagsMateria.join(" or ")}) and #${TAGS_FACULTAD.self}/${TAGS_FACULTAD.materia}`)
                .filter(materia => !materia[DATOS_MATERIA.equivalencia]);
        }

        if (materias.length > 0 && materiasPorResumen.every(materia => materia.file.path != materias.first().file.path)) {
            materiasPorResumen = [ ...materiasPorResumen, materias.first()];
        }

        if (materiasPorResumen.length == 0) 
            throw error.Quit("De alguna forma hay materias que no llevan a ninguna carrera");

        let materia = materiasPorResumen.first();
        if (materiasPorResumen.length > 1) {
            materia = await preguntar.suggester(
                tp, materiasPorResumen.map(carrera => carrera.file.name), materiasPorResumen,
                "En que materia queres crear el tema de la materia?",
                error.Quit("No se ingreso que materia se usa para crear el resumen")
            )
        }

        return await seccionResumen.crear(materia);
    }

    return await seccionCarrera.crear();
    */
}

module.exports = (tp) => ({
    elegirYCrear: crear.bind(null, tp),
    crearArbol: crearEstructuraFacultad.bind(null, tp),
});