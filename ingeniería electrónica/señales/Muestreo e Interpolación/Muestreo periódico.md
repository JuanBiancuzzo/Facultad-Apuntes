...
Vemos que la transformada de Fourier de $x(n)$ es [[Función periódica|periódica]] como debe ser y que está directamente relacionada con la transformada $X_c(j\omega)$ de la [[Señal#^continua|señal de tiempo continuo]] origina $x_c(t)$

Está claro que si [[Señal de banda limitada|señal limitada]] $X_c(j\omega) = 0, ~~ \forall |\omega| \ge W$ y $2W < \omega_s$ [[ingeniería electrónica/señales/Muestreo e Interpolación/Aliasing|no tenemos aliasing]] y que $$ X\left( e^{j\Omega} \right) = \frac{1}{T} X_c\left( j \frac{\Omega}{T} \right), ~~~ \Omega \in [-\pi, ~\pi) $$
