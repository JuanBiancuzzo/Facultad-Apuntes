...
# Definición
---
La intención del [[ingeniería en informática/ingenieria de software 1/Ingeniería de software/Modelo|modelo]] es poder aplicar [[Principio de superposición|superposición]] de una fuente de [[Tensión|tensión]] aplicada sobre el [[Diodo de Juntura PN|diodo]] ($v_D$), donde tenemos una parte continua ($V_D$) y una pequeña señal dependiente del tiempo ($v_d(t)$) $$ v_D(t) = V_D + v_d(t) $$
...
Como el [[Diodo]] es [[Función lineal|no lineal]], vemos que $i_D(t) \ne i_{D1} + i_{D2}$, por lo que este [[ingeniería en informática/ingenieria de software 1/Ingeniería de software/Modelo|modelo]] intenta linealizar la respuesta [[Entorno|entorno]] a un punto de reposo ($v_D = V_D$)
...
Entonces, como dijimos $$ \begin{matrix} 
    i_d = \frac{\partial i_D}{\partial v_D} \Biggm|_{v_D=V_D} v_d = g_d ~ v_d \\
    g_d = \frac{I_{Dq}}{V_T}
\end{matrix} $$
...
* $C_j \equiv$ [[Capacidad de juntura|Capacidad de juntura]] $[C_j] = F$
* $C_d \equiv$ [[Capacidad de difusión|Capacidad de difusión]] $[C_d] = F$
...
Pero esta inecuación no tiene solución, por lo que podemos encontrar una [[ingeniería en informática/discreta/Relaciones/Cota]] a este error, aplicando la [[Serie de Taylor|serie de Taylor]], ya que notemos que $$ \underbrace{
...
En [[Convención de signos para la tensión de polarización#Polarización directa|directa]] tenemos que $$ \begin{align} 
	...
	v_d &< 0.2 \cdot V_{th}
\end{align} $$
Que a [[Temperatura|temperatura]] ambiente se obtiene $v_d < 5.2 ~mV$ pero en la práctica por las aproximaciones dadas al encontrar la [[Corriente del Diodo|corriente del diodo]], termina valiendo que $$ |v_d| < 10 ~mV ~ \text{(pico)} $$