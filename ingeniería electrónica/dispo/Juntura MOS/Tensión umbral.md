...
Se define la situación umbral cuando $n(0) = N_a$. Por lo que superando el umbral, no se puede despreciar la contribución de los [[Electrón|electrones]] a la [[Campo eléctrico]].
...
* Utilizar la [[Campo eléctrico]] de vaciamiento (despreciar la concentración de [[Electrón|electrones]] para tensión umbral)
...
Segundo, calculamos la caída de [[Tensión|potencial]] en el óxido para la tensión umbral $$ V_{ox}(V_T) = E_{ox}(V_T) ~ t_{ox} = \frac{q ~ N_a ~ x_d(V_T)}{\varepsilon_{ox}} ~ t_{ox} $$
...
Ahora encontrando la caída de potencial en el óxido queda $$ V_{ox}(V_T) = \frac{q ~ N_a ~ x_d(V_T)}{\varepsilon_{ox}} ~ t_{ox} = \gamma \sqrt{-2 \phi_o} $$ donde $\gamma$ es el [[Body factor coefficient|Body factor coefficient]]

Finalmente, sumamos las caídas de potencial en toda la estructura $$ V_T + \phi_B = V_B(V_T) + V_{ox}(V_T) = -2\phi_p + \gamma \sqrt{-2 \phi_p} $$
Siendo $\phi_B = -V_{FV}$ ([[Tensión de Flatband|tensión de Flatband]]), se despeja $V_T$: $$ V_T = V_{FB} - 2\phi_p + \gamma \sqrt{-2 \phi_p} $$
## Para un MOSFET
---
...
Si aplicamos la tensión $V_{BS}$, la misma afecta la caída del [[Tensión|potencial]] en el [[Semiconductor]], dando que $V_B = -2 \phi_p$ pasa a ser $V_B = -2 \phi_p - V_{BS}$
