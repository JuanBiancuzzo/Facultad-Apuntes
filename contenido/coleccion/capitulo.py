...
class Capitulo(Dato):
    ...
    def parsear(cls, archivo: Archivo) -> list[Dato]:
        ...
        if etapa is None:
            raise ErrorParseo(
                f"La etapa de un capitulo del libro {archivo.metadata.nombre} no es valida {archivo.extra['etapa']}"
            )
        ...
        for extra_capitulo in info_capitulos:
            ...
            for num_guia in (int(num) for num in extra_capitulo.get("guias", [])):
    ...
    def _obtener_clave(cls, clave_libro: Clave, clave_ref_capitulo: Clave) -> Clave:
        return Clave.de_texto(
            TipoNodo.CAPITULO, f"{clave_libro}<:>{clave_ref_capitulo}"
        )
    ...
    def insertar_datos(
        ...
        try:
            ...
            id_capitulo = Tabla.insertar(
                ...
                self.etapa.value,
                id_resumen,
                dependencias[self.clave_libro],
                dependencias[self.clave_ref_capitulo],
            )

        except Exception as err:
            raise ErrorInsertar(
                f"Al insertar coleccion de capitulo de un libro, con clave de libro: {self.clave_libro}",
                err,
            )

        if id_capitulo is None:
            raise ErrorIdNoGenerado("El capitulo insertado no tiene id")
