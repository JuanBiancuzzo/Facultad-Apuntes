...
# Definición
---
Considere la [[Función|función]] $f : U \subseteq \mathbb{R}^n \to \mathbb{R}$ definida en el [[Conjunto abierto|conjunto abierto]] $U$ de $\mathbb{R}^n$ Sea $\vec{x} \in U$. Si cualquier [[Disco abierto|disco abierto]] B con centro en $\vec{x}$  contiene puntos $x \in B$ tales que $f(x) - f(\vec{x})$ es positivo y puntos $y \in B$ tales que $f(y) - f(\vec{x})$ es negativos, se dice que $\vec{x}$ es un punto de ensilladura de la función $f$

Esto se puede pensar como que para una [[Derivada direccional|derivada direccional]] es un [[Punto critico|punto critico]] siendo este el [[Máximo|máximo]], y para otra dirección un punto critico [[Mínimo|mínimo]]
