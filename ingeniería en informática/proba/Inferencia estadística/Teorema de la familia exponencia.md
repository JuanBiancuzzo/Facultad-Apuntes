---
dia: 2023-01-23
tags:
  - carrera/ingeniería-electrónica/proba/Inferencia-estadística
  - carrera/ingeniería-en-informática/proba/Inferencia-estadística
  - nota/facultad
vinculoFacultad:
  - tema: Inferencia estadística
    capitulo: 10
    materia: Probabilidad y estadística B
    carrera: Ingeniería en informática
---
# Definición
---
Una [[Familia exponencial]] de $k$ parámetros tiene como [[ingeniería en informática/proba/Inferencia estadística/Estadístico suficiente]] para $\theta \in \mathbb{R}^k$ al vector $$ T = \bigg( r_1(\underline{X}), ~r_2(\underline{X}), ~\cdots, ~r_k(\underline{X})  \bigg) $$
Ahora, si $X_1, \cdots, X_n$ es una [[Muestra aleatoria]] de una distribución perteneciente a una [[Familia exponencial]] a $k$ parámetros, entonces el [[Vector aleatorio]] $\underline{X}$ también tiene una distribución perteneciente a una [[Familia exponencial]] a $k$ parámetros, y el [[ingeniería en informática/proba/Inferencia estadística/Estadístico suficiente]] para $\theta$ basado en la [[Muestra aleatoria]] será $$ T = \bigg( \sum_{i = 1}^n r_1(\underline{X}), ~\sum_{i = 1}^n r_2(\underline{X}), ~\cdots, ~\sum_{i = 1}^n r_k(\underline{X})  \bigg) $$