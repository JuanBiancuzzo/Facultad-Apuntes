...
# Definición
---
> [!teorema]+ Teorema 1.2.18 (Algoritmo de división)
...
> [!observacion]+ Observación 1.2.19 
> Si $0 \le a < |d|$, entonces $a = 0 \cdot d + a$ implica $k = 0$ y $r = r_d(a) = a$ pues $a$ cumple la condición que tiene que cumplir el resto (se aplica la unicidad del cociente y el resto)
^obs-1-2-19

## Divisibilidad y resto
---
> [!observacion]+ Observación 1.2.20 (Divisibilidad y resto)
> Sean $a,~ d \in \mathbb{Z}$, $d \ne 0$. Entonces $$ d_d(a) = 0 \iff d \mid a \iff a \equiv 0 ~ (\text{mod} ~ d) $$
^obs-1-2-20

## Congruencia y resto
---
> [!proposicion]+ Proposición 1.2.21 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces $a \equiv r_d(a) ~ (\text{mod} ~ d), ~~ \forall a \in \mathbb{Z}$
> 
> > [!demostracion]- Demostración
> > Pues $a = k ~ d + r_d(a) \implies a - r_d(a) = k ~ d \implies a \equiv r_d(a) ~ (\text{mod} ~ d)$
^prop-1-2-21

> [!proposicion]+ Proposición 1.2.22 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces $a \equiv r ~ (\text{mod} ~ d) ~ \text{con} ~ 0 \le r < |d| \implies r = r_d(a)$
> 
> > [!demostracion]- Demostración
...
> [!proposicion]+ Proposición 1.2.23 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces $r_1 \equiv r_2 ~ (\text{mod} ~ d) ~ \text{con} ~ 0 \le r_1,~ r_2 < |d| \implies r_1 = r_2$
> 
> > [!demostracion]- Demostración
...
> [!proposicion]+ Proposición 1.2.24 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces $a \equiv b ~ (\text{mod} ~ d) \iff r_d(a) = r_d(b)$
> 
> > [!demostracion]- Demostración
...
## Tablas de Restos
---
> [!corolario]+ Corolario 1.2.25 (Tablas de Resto) - [[licenciatura en ciencias matemáticas/algebra 1/Números enteros/Algoritmo de división#^teo-1-2-18|Teorema 1.2.18 (Algoritmo de división)]]
