...
---
> [!proposicion]+ Proposición 1.2.11 (La congruencia es una relación de equivalencia)
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Sea $\mathcal{R}$ la [[Relación|relación]] en $\mathbb{Z}$ dada por $$ a \mathcal{R} b \iff a \equiv b ~ (\text{mod} ~ d), ~~~ \forall a,~ b \in \mathbb{Z} $$ Entonces $\mathcal{R}$ es una [[Relación de equivalencia|relación de equivalencia]] 
> 
...
## Propiedades
---
> [!proposicion]+ Proposición 1.2.12 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces $\forall a_1,~ a_2,~ b_1,~ b_2 \in \mathbb{Z}$ $$ a_1 \equiv b_1 ~ (\text{mod} ~ d) ~~ \text{y} ~~ a_2 \equiv b_2 ~ (\text{mod} ~ d) ~~ \text{y} \implies a_1 + a_2 \equiv b_1 + b_2 ~ (\text{mod} ~ d) $$
> 
> > [!demostracion]- Demostración
...
> [!proposicion]+ Proposición 1.2.13 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces 
> Para todo $n \in \mathbb{N}$, $a_1,~ \cdots,~ a_n,~ b_1,~ \cdots,~ b_n \in \mathbb{Z}$ $$ \begin{cases} 
...
> [!proposicion]+ Proposición 1.2.14 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces $\forall a,~ b,~ c \in \mathbb{Z}$, $$ a \equiv b ~ (\text{mod} ~d) \implies c ~ a \equiv c ~ b ~ (\text{mod} ~d) $$
> 
> > [!demostracion]- Demostración
...
> [!proposicion]+ Proposición 1.2.15 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces $\forall a_1,~ a_2,~ b_1,~ b_2 \in \mathbb{Z}$, $$ a_1 \equiv b_1 ~ (\text{mod} ~ d) ~~ \text{y} ~~ a_2 \equiv b_2 ~ (\text{mod} ~ d) ~~ \text{y} \implies a_1 ~ a_2 \equiv b_1 ~ b_2 ~ (\text{mod} ~ d) $$
> 
> > [!demostracion]- Demostración
...
> [!proposicion]+ Proposición 1.2.16 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces Para todo $n \in \mathbb{N}$, $a_1,~ \cdots,~ a_n,~ b_1,~ \cdots,~ b_n \in \mathbb{Z}$ $$ \begin{cases} 
>     a_1 \equiv b_1 ~ (\text{mod} ~ d) \\
>     ~~~~~~\vdots \\
>     a_n \equiv b_n ~ (\text{mod} ~ d) \\
>     \end{cases} \implies a_1 ~ \cdots ~ a_n \equiv b_1 ~ \cdots ~ b_n ~ (\text{mod} ~ d) $$
...
> [!proposicion]+ Proposición 1.2.17 
> Sea $d \in \mathbb{Z}$, $d \ne 0$. Entonces $\forall a,~ b \in \mathbb{Z}$, $n \in \mathbb{N}$, $$ a \equiv b ~ (\text{mod} ~ d) \implies a^n \equiv b^n ~ (\text{mod} ~ d) $$
> 
> > [!demostracion]- Demostración
