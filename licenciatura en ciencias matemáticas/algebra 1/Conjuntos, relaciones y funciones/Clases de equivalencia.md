...
Pero luego, todo elemento $z' \in [x]$ satisface $z' \mathcal{R} x$, y como $x \mathcal{R} y$, se tiene $z' \mathcal{R} y$, o sea $z' \in [y]$. Es decir, hemos probado que $[x] \subseteq [y]$, y del mismo modo se prueba $[y] \subseteq [x]$. Por lo tanto 
