...
### Definición
---
Dado dos [[Conjunto|conjuntos]] $A$ y $B$, se define la función en [[Álgebra de conjuntos|álgebra de conjuntos]] como $$ A \cap B \equiv A \cdot B $$
...
\begin{document} 
	\begin{tikzpicture}
		...
		\path (ab) -- ++(0, 2.4) 
			node[font=\bfseries, black, scale=1.75] {$A \cdot B$};
	\end{tikzpicture}
\end{document}
```
