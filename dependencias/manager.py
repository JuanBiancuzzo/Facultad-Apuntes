...
class ManagerDependencias:
    ...
    def guardar_nodo(
        ...
        clave: Clave,
        dato: Dato,
        clave_dependencias: list[Clave],
    ) -> None:
        if clave in self.datos:
            if self.datos[clave] == dato:
                return

            loggear(
                LoggerNivel.ERROR,
                f"Ya existe la clave: {clave} en los datos del manager {self.datos[clave]} = {dato}?",
            )
            return
        ...
        clave_filtrada = list(
            filter(
                lambda clave_dependencia: not self.preexiste(cursor, clave_dependencia),
                clave_dependencias,
            )
        )
    ...
    def obtener_depencencia(
        self, cursor: sql.Cursor, claves: list[Clave]
    ) -> dict[Clave, int]:
        dependencias = {}
        ...
        for clave in claves:
            ...
            if resultado is None:
                raise Exception(
                    f"Imposible obtener dependencia para clave de tipo: {clave.tipo} y hash:{clave.hash}"
                )
            dependencias[clave] = resultado[0]

        if len(claves) != len(dependencias.keys()):
            raise Exception(
                f"Se esperaba obtener {len(claves)} claves, pero se obtuvo menos"
            )

        return dependencias

    def actualizar_nodos(
        self, cursor: sql.Cursor, nodo_insertado: Nodo | None
    ) -> list[Dato]:
        ...
        for clave in map(Clave.de_tupla, cursor.fetchall()):
            try:
                ...
                loggear(
                    LoggerNivel.FATAL,
                    f"Al intentar insertar {clave} se vio que no existia",
                )
                raise e
    ...
    def close(self):
        if len(self.datos) > 0:
            ...
            def imprimir_item_pendiente(clave: Clave, dato: Dato) -> str:
                ...
                tabla_cadena = map(
                    lambda item: (Clave(item[0], item[1]), item[2] == 1),
                    cursor.fetchall(),
                )
                return f"Al {dato} le faltaban: {list(map(imprimir, tabla_cadena))}"

            items_pendientes = map(
                lambda item: imprimir_item_pendiente(*item), self.datos.items()
            )
            loggear(
                LoggerNivel.WARN,
                f"Cerrando el manager, datos pendientes: \n\t{'\n\t'.join(items_pendientes)}",
            )
            cursor.close()
